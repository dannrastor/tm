...
double StopManager::GetDistance(const Stop& lhs, const Stop& rhs) const {
    ...
    double r_lon = lhs.longitude * 180 / 3.1415926535; 
...
void StopManager::AddStop(const AddQuery& query) {
    Stop stop;
    stop.name = query.GetName();
    stop.latitude = ConvertToFloat(query.GetContents()[0]);
    stop.longitude = ConvertToFloat(query.GetContents()[1]);
